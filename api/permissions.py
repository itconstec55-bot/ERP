...
class ModulePermission(BasePermission):
    ...
    def has_permission(self, request, view):
        if request.user.is_staff:
            return True
        lookup = self.MODULE_MAP.get(view.basename or view.base_name)
        if not lookup:
            return False
        action = self.ACTION_MAP.get(view.action, 'view')
        from access_control.resolver import has_permission as check_perm
        return check_perm(request.user, lookup[0], action)
