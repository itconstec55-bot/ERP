...
class SalesOrder(models.Model):
    STATUS_CHOICES = [('draft', 'مسودة'), ('confirmed', 'مؤكد'), ('invoiced', 'مفوتر'), ('cancelled', 'ملغي')]

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    order_number = models.CharField(max_length=50, unique=True, blank=True, verbose_name='رقم أمر البيع')
    ...
    def remaining_to_invoice(self):
        return sum((line.quantity - line.invoiced_quantity) for line in self.lines.all())


class SalesOrderLine(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    order = models.ForeignKey(SalesOrder, on_delete=models.CASCADE, related_name='lines', verbose_name='أمر البيع')
    ...
    unit_price = models.DecimalField(max_digits=20, decimal_places=10, default=0, verbose_name='سعر البيع')
    invoiced_quantity = models.DecimalField(max_digits=20, decimal_places=10, default=0, verbose_name='الكمية المفوترة')
    notes = models.TextField(blank=True, null=True, verbose_name='ملاحظات')
