...
from purchases.models import ProductCategory, Product
...
class SalesInvoiceCalculationTest(TestCase):
    def setUp(self):
        ...
        revenue_type = AccountType.objects.update_or_create(code='revenue', defaults={'name': 'إيرادات', 'account_type': 'revenue'})[0]
    ...
    def test_multi_line_invoice(self):
        invoice = SalesInvoice.objects.create(
            invoice_number='INV-002', customer=self.customer,
            date=date.today(),
            created_by=self.user,
        )
        SalesInvoiceLine.objects.create(
        ...
        SalesInvoiceLine.objects.create(
            invoice=invoice, product=self.product,
            quantity=Decimal('3'), unit_price=Decimal('80.00'),
        )
        invoice.calculate_totals()
    ...
    def test_line_total_price(self):
        invoice = SalesInvoice.objects.create(
            invoice_number='INV-003', customer=self.customer,
            date=date.today(),
            created_by=self.user,
        )
        line = SalesInvoiceLine.objects.create(
            invoice=invoice, product=self.product,
            quantity=Decimal('5'), unit_price=Decimal('40.00'),
        )
