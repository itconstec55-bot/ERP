...
from deployment.config.settings import (
    ALERTS_ENABLED, ALERTS_METHOD, ALERTS_LOG_DIR,
    ALERTS_EMAIL_SMTP, ALERTS_EMAIL_PORT,
    ALERTS_EMAIL_USER, ALERTS_EMAIL_PASS,
    ALERTS_EMAIL_TO, ALERTS_EMAIL_FROM,
...
def alert_server_down(host, port):
    AlertManager().send(
        "السيرفر متوقف",
        f"السيرفر على {host}:{port} لا يستجيب للطلبات",
        AlertLevel.CRITICAL,
    )
...
def alert_server_restarted(attempts):
    AlertManager().send(
        "إعادة تشغيل السيرفر",
        f"تم إعادة تشغيل السيرفر تلقائياً (محاولة رقم {attempts})",
        AlertLevel.WARNING,
    )
...
def alert_health_check_failed(failures, threshold):
    AlertManager().send(
        "فحص الحالة فشل",
        f"فشل فحص الحالة ({failures}/{threshold}) - سيتم إعادة التشغيل قريباً",
        AlertLevel.WARNING,
    )
...
def alert_update_available(info):
    AlertManager().send(
        "تحديث متاح",
        f"تم اكتشاف تحديث جديد: {json.dumps(info, default=str)[:200]}",
        AlertLevel.INFO,
    )
...
def alert_update_applied(version=None):
    AlertManager().send(
        "تم تطبيق التحديث",
        f"تم تطبيق التحديث{' (' + version + ')' if version else ''} بنجاح",
        AlertLevel.INFO,
    )
...
def alert_disk_low(space_mb):
    AlertManager().send(
        "مساحة القرص منخفضة",
        f"المساحة المتاحة على القرص: {space_mb}MB فقط",
        AlertLevel.WARNING,
    )
