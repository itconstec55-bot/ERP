...
from deployment.config.settings import (
    ...
    LOGS_DIR,
    PYTHON_EXE,
    UPDATES_CHECK_INTERVAL,
    UPDATES_ENABLED,
    UPDATES_GIT_BRANCH,
    UPDATES_GIT_REPO,
    UPDATES_INSTALL_DEPS,
    UPDATES_REQUIRE_RESTART,
    UPDATES_RUN_MIGRATIONS,
    UPDATES_SOURCE,
)
...
def _save_state(state):
    ...
    UPDATE_STATE_FILE.write_text(json.dumps(state, indent=2, default=str), encoding='utf-8')
...
class GitUpdater:
    ...
    def _run(self, cmd, cwd=None):
        result = subprocess.run(cmd, cwd=cwd or self.repo_path, capture_output=True, text=True, timeout=120)
    ...
    def check_for_updates(self):
        ...
        ok, remote_hash, _ = self._run(['git', 'rev-parse', f'origin/{self.branch}'])
        if not ok:
            return None, 'Cannot get remote HEAD'

        if local_hash != remote_hash:
            ok, log_out, _ = self._run(['git', 'log', '--oneline', f'HEAD..origin/{self.branch}'])
...
class FileUpdater:
    ...
    def check_for_updates(self):
        ...
        latest = zips[0]
        return {'available': True, 'file': str(latest), 'size': latest.stat().st_size}, None
    ...
    def _backup_current(self):
        ...
        exclude = {'.git', '__pycache__', 'venv', 'env', 'deployment/backups', 'deployment/logs'}
...
class UpdateManager:
    ...
    def _install_dependencies(self):
        ...
        ok = (
            subprocess.run(
                [PYTHON_EXE, '-m', 'pip', 'install', '-r', str(req_file), '-q'], capture_output=True, timeout=300
            ).returncode
            == 0
        )
        if ok:
...
def main():
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler(str(Path(LOGS_DIR) / 'updates.log'), encoding='utf-8')],
    )
