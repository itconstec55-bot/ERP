...
class HealthMonitor:
    ...
    def _should_restart(self):
        ...
        if self.state['total_restarts'] >= RESTART_MAX_ATTEMPTS:
            logger.error(f'Max restarts ({RESTART_MAX_ATTEMPTS}) reached. Manual intervention required.')
            return False
        if self.state['last_restart']:
            elapsed = (datetime.now() - datetime.fromisoformat(self.state['last_restart'])).total_seconds()
            if elapsed < self.state.get('current_backoff', RESTART_COOLDOWN):
                return False
        return True

    def _do_restart(self):
        logger.warning(f'Restarting server (attempt {self.state["total_restarts"] + 1})')
        ...
        if not success:
            self.state['current_backoff'] = (
                self.state.get('current_backoff', RESTART_COOLDOWN) * RESTART_BACKOFF_MULTIPLIER
            )
    ...
    def run_once(self):
        ...
        if ok:
            ...
            if self.state['total_restarts'] > 0:
                elapsed = (datetime.now() - datetime.fromisoformat(self.state['last_healthy'])).total_seconds()
                if elapsed > RESTART_COOLDOWN_RESET_AFTER:
                    self.state['total_restarts'] = max(0, self.state['total_restarts'] - 1)
        else:
            self.state['consecutive_failures'] += 1
            logger.warning(f'Consecutive failures: {self.state["consecutive_failures"]}/{HEALTH_CHECK_FAIL_THRESHOLD}')
            if self.state['consecutive_failures'] >= HEALTH_CHECK_FAIL_THRESHOLD and self._should_restart():
                self._do_restart()
...
def main():
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(str(Path(LOGS_DIR) / 'health_check.log'), encoding='utf-8'),
        ],
    )
