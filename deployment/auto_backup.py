...
def _save_state(state):
    ...
    BACKUP_STATE_FILE.write_text(json.dumps(state, indent=2, default=str), encoding='utf-8')
...
def full_backup():
    ...
    logger.info(f'Backup complete: db={"OK" if db_result else "FAIL"}, files={"OK" if files_result else "FAIL"}')
    return success
...
def run_forever():
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(), logging.FileHandler(str(Path(LOGS_DIR) / 'backup.log'), encoding='utf-8')],
    )
