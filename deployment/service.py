...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(str(logs_dir / 'service.log'), encoding='utf-8'),
        ],
    )
...
def run_health_check():
    ...
    while not shutdown_event.is_set():
        ...
        if consecutive_failures >= threshold:
            logger.error(f'Health check: {consecutive_failures} consecutive failures')
...
def main():
    ...
    threads = []
    threads.append(threading.Thread(target=run_django_server, name='django', daemon=True))
    if HEALTH_CHECK_ENABLED:
        threads.append(threading.Thread(target=run_health_check, name='health', daemon=True))
    if UPDATES_ENABLED:
        threads.append(threading.Thread(target=run_updater, name='updater', daemon=True))
    if BACKUPS_ENABLED:
        threads.append(threading.Thread(target=run_backup, name='backup', daemon=True))
