...
class PaymentReceipt(models.Model):
    ...
    def create_journal_entry(self):
        ...
        if self.receipt_type == 'receipt':
            ...
            debit_account = self.safe.account if self.safe else (self.bank.account if self.bank else JournalEntryService.get_account('1500'))
            credit_account = self.customer.account if self.customer and self.customer.account else JournalEntryService.get_account('1100')
        else:
            desc = f'سند دفع - {self.supplier.name if self.supplier else ""} - {self.receipt_number}'
            debit_account = self.supplier.account if self.supplier and self.supplier.account else JournalEntryService.get_account('3100')
            credit_account = self.safe.account if self.safe else (self.bank.account if self.bank else JournalEntryService.get_account('1500'))
