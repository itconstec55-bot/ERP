...
class WarehouseProduct(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    warehouse = models.ForeignKey(Warehouse, on_delete=models.CASCADE, related_name='products', verbose_name='المخزن')
    product = models.ForeignKey(
        Product, on_delete=models.PROTECT, related_name='warehouse_stock', verbose_name='المنتج'
    )
    quantity = models.DecimalField(max_digits=10, decimal_places=2, default=0, verbose_name='الكمية الحالية')
    minimum_quantity = models.DecimalField(max_digits=10, decimal_places=2, default=0, verbose_name='الحد الأدنى')
    maximum_quantity = models.DecimalField(max_digits=10, decimal_places=2, default=0, verbose_name='الحد الأقصى')
...
class StockMovement(models.Model):
    ...
    warehouse = models.ForeignKey(Warehouse, on_delete=models.CASCADE, related_name='movements', verbose_name='المخزن')
    ...
    product = models.ForeignKey(
        Product, on_delete=models.PROTECT, related_name='stock_movements', verbose_name='المنتج'
    )
    ...
    notes = models.TextField(blank=True, null=True, verbose_name='ملاحظات')
    performed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, verbose_name='نفّذ الحركة')
...
class InventoryCostLayer(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    product = models.ForeignKey(Product, on_delete=models.PROTECT, related_name='cost_layers', verbose_name='المنتج')
    warehouse = models.ForeignKey(
        Warehouse, on_delete=models.CASCADE, related_name='cost_layers', verbose_name='المخزن'
    )
    reference_number = models.CharField(max_length=100, blank=True, null=True, verbose_name='رقم المرجع')
    quantity_remaining = models.DecimalField(
        max_digits=20, decimal_places=10, default=0, verbose_name='الكمية المتبقية'
    )
    unit_cost = models.DecimalField(max_digits=20, decimal_places=10, default=0, verbose_name='تكلفة الوحدة')
    total_cost = models.DecimalField(max_digits=30, decimal_places=10, default=0, verbose_name='التكلفة الإجمالية')
    date = models.DateField(verbose_name='التاريخ')
    is_active = models.BooleanField(default=True, verbose_name='نشط')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def consume_fifo(cls, product, warehouse, quantity, reference_number=None, date=None):
        ...
        layers = (
            ...
            .filter(product=product, warehouse=warehouse, is_active=True, quantity_remaining__gt=0)
