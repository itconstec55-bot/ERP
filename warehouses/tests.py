...
def _make_product():
    ...
    return Product.objects.create(
        code='PROD-1', name='منتج اختبار', category=cat, unit=uom,
...
class TestStockMovement:
    def test_create_out(self):
        ...
        movement = StockMovement.objects.create(
            movement_number='SM-001', movement_type='out', warehouse=w, product=p,
            quantity=Decimal('10'), unit_cost=Decimal('50'), date=date.today(),
        )
    ...
    def test_str(self):
        ...
        p = _make_product()
        movement = StockMovement.objects.create(
            movement_number='SM-002', movement_type='in', warehouse=w, product=p,
            quantity=Decimal('20'), unit_cost=Decimal('30'), date=date.today(),
        )
