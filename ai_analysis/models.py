...
class ErrorLog(models.Model):
    SEVERITY_CHOICES = [('low', 'منخفض'), ('medium', 'متوسط'), ('high', 'عالي'), ('critical', 'حرج')]
    STATUS_CHOICES = [
        ('detected', 'تم الكشف'),
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='detected', db_index=True, verbose_name='الحالة'
    )
    ...
    resolved_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.SET_NULL, verbose_name='تم الحل بواسطة'
    )
...
class Solution(models.Model):
    PRIORITY_CHOICES = [(1, 'الأولى'), (2, 'الثانية'), (3, 'الثالثة')]
    ...
    applied_by = models.ForeignKey(
        User, null=True, blank=True, on_delete=models.SET_NULL, verbose_name='تم التطبيق بواسطة'
    )
    created_at = models.DateTimeField(auto_now_add=True)
