...
class AccountingErrorDetector:
                ...
                self.errors.append(
                    {
                        ...
                        'entry_date': entry.date,
                        'journal_entry_id': entry.id,
                        'raw_data': {'duplicate_of': seen[key], 'entry_number': entry.entry_number},
                    }
                )
            else:
    ...
    def detect_posting_duplicates(self):
        ...
        sales_entries = (
            SalesInvoice.objects.filter(is_posted=True, journal_entry__isnull=False)
        ...
        purchase_entries = (
            PurchaseInvoice.objects.filter(is_posted=True, journal_entry__isnull=False)
            .values('journal_entry')
            .annotate(count=Count('id'))
            .filter(count__gt=1)
        )
    ...
    def detect_orphan_entries(self):
        ...
        for inv in PurchaseInvoice.objects.filter(journal_entry__isnull=False).values_list(
