...
from common.accounting_service import AccountNotFoundError, JournalEntryService, UnbalancedEntryError
from common.exceptions import AccountingError, InsufficientStockError
...
from purchases.models import Product, ProductCategory, PurchaseInvoice, PurchaseInvoiceLine, Supplier, UnitOfMeasure
...
class JournalEntryServiceTest(TestCase):
    ...
    def setUp(self):
        ...
        self.supplier = Account.objects.create(code='3100', name='موردون', account_type=self.liab_type)
    ...
    def test_create_balanced_entry_success(self):
        ...
        entry = JournalEntryService.create_entry(
            entry_type='general', date='2024-01-15', description='قيد اختبار', lines=lines, created_by=self.user
        )
    ...
    def test_create_unbalanced_entry_raises_error(self):
        ...
        with self.assertRaises(UnbalancedEntryError):
            JournalEntryService.create_entry(
                entry_type='general', date='2024-01-15', description='قيد غير متوازن', lines=lines, created_by=self.user
            )
...
class AccountModelTest(TestCase):
    ...
    def test_account_opening_balance_validator(self):
        ...
        account = Account.objects.create(
            code='1200', name='البنك', account_type=self.acc_type, opening_balance=Decimal('5000.00')
        )
        self.assertEqual(account.opening_balance, Decimal('5000.00'))

        # صفر صحيح
        account2 = Account.objects.create(
            code='1201', name='صندوق', account_type=self.acc_type, opening_balance=Decimal('0')
        )
        ...
        account3 = Account.objects.create(
            code='1202', name='سالب', account_type=self.acc_type, opening_balance=Decimal('-100.00')
        )
    ...
    def test_account_current_balance_validator(self):
        ...
        account = Account.objects.create(code='1300', name='النقدية', account_type=self.acc_type)
...
class JournalEntryLineModelTest(TestCase):
    ...
    def test_journal_entry_line_validators(self):
        ...
        line = JournalEntryLine(
            journal_entry=self.entry, account=self.cash, debit=Decimal('1000.00'), credit=Decimal('0.00')
        )
        line.full_clean()  # يجب أن يمر

        # صحيح - دائن فقط
        line2 = JournalEntryLine(
            journal_entry=self.entry, account=self.revenue, debit=Decimal('0.00'), credit=Decimal('1000.00')
        )
        line2.full_clean()  # يجب أن يمر

        # خاطئ - مدين ودائن معاً
        line3 = JournalEntryLine(
            journal_entry=self.entry, account=self.cash, debit=Decimal('1000.00'), credit=Decimal('500.00')
        )
        with self.assertRaises(ValidationError):
            line3.full_clean()
        ...
        line4 = JournalEntryLine(
            journal_entry=self.entry, account=self.cash, debit=Decimal('0.00'), credit=Decimal('0.00')
        )
        with self.assertRaises(ValidationError):
            line4.full_clean()
    ...
    def test_journal_entry_line_both_debit_credit_rejected(self):
        ...
        line = JournalEntryLine(
            journal_entry=self.entry, account=self.cash, debit=Decimal('100.00'), credit=Decimal('200.00')
        )
        with self.assertRaises(ValidationError):
            line.full_clean()
...
class PurchaseInvoiceModelTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpass123')
        self.supplier = Supplier.objects.create(code='SUP001', name='مورد اختبار', supplier_type='company')
        self.category = ProductCategory.objects.create(code='CAT001', name='فئة اختبار')
        self.unit = UnitOfMeasure.objects.create(code='PCS', name='قطعة')
    ...
    def test_purchase_invoice_line_calculation(self):
        ...
        invoice = PurchaseInvoice.objects.create(
            invoice_number='PI-001', supplier=self.supplier, date='2024-01-15', created_by=self.user
        )
...
class SalesInvoiceModelTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpass123')
        self.customer = Customer.objects.create(code='CUS001', name='عميل اختبار', customer_type='company')
        self.category = ProductCategory.objects.create(code='CAT001', name='فئة اختبار')
        self.unit = UnitOfMeasure.objects.create(code='PCS', name='قطعة')
        self.product = Product.objects.create(
            code='PROD001',
    ...
    def test_sales_invoice_line_calculation(self):
        ...
        invoice = SalesInvoice.objects.create(
            invoice_number='SI-001', customer=self.customer, date='2024-01-15', created_by=self.user
        )
...
class FinancialValidatorIntegrationTest(TestCase):
    ...
    def test_validate_invoice_payment(self):
        ...
        FinancialValidator.validate_invoice_payment(
            payment_amount=Decimal('500.00'), remaining_amount=Decimal('1000.00')
        )

        with self.assertRaises(ValidationError):
            FinancialValidator.validate_invoice_payment(
                payment_amount=Decimal('1500.00'), remaining_amount=Decimal('1000.00')
            )
...
class ExceptionHandlingTest(TestCase):
    ...
    def test_insufficient_stock_error(self):
        ...
        error = InsufficientStockError(product_name='منتج', available=Decimal('5.00'), requested=Decimal('10.00'))
    ...
    def test_sequential_posting_simulation(self):
        ...
        for i in range(10):
            ...
            JournalEntryLine.objects.create(
                journal_entry=entry, account=self.cash, debit=Decimal('100.00'), credit=Decimal('0.00')
            )
            JournalEntryLine.objects.create(
                journal_entry=entry, account=self.revenue, debit=Decimal('0.00'), credit=Decimal('100.00')
            )
...
class ModelValidationIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.purchases_acc = Account.objects.create(code='1300', name='مشتريات', account_type=self.exp_type)
        self.vat_acc = Account.objects.create(code='1350', name='ضريبة القيمة المضافة', account_type=self.acc_type)
        self.wt_acc = Account.objects.create(code='2130', name='الخصم والتحصيل', account_type=self.liab_type)
        self.discount_acc = Account.objects.create(code='4300', name='خصم مكتسب', account_type=self.rev_type)
        self.supplier_acc = Account.objects.create(code='3100', name='الموردون', account_type=self.liab_type)
        self.cogs_acc = Account.objects.create(code='5100', name='تكلفة البضاعة', account_type=self.exp_type)
        self.inventory_acc = Account.objects.create(code='1200', name='المخزون', account_type=self.acc_type)
        self.customer_acc = Account.objects.create(code='1101', name='العملاء', account_type=self.acc_type)

    def test_full_invoice_creation_and_posting(self):
        """اختبار دورة حياة كاملة: إنشاء فاتورة وترحيلها"""
        from purchases.models import Product, ProductCategory, Supplier, UnitOfMeasure

        supplier = Supplier.objects.create(code='SUP001', name='مورد اختبار', supplier_type='company')
        ...
        PurchaseInvoiceLine.objects.create(
            invoice=invoice, product=product, quantity=Decimal('10.0000000000'), unit_price=Decimal('100.0000000000')
        )
        invoice.calculate_totals()
...
class ServiceLayerTest(TestCase):
    ...
    def test_service_creates_balanced_entry(self):
        ...
        entry = JournalEntryService.create_entry(
            entry_type='general', date='2024-01-15', description='اختبار خدمة', lines=lines, created_by=self.user
        )
        self.assertTrue(entry.is_posted)
        self.assertEqual(entry.total_debit, Decimal('1000.00'))
    ...
    def test_service_validates_account_existence(self):
        ...
        with self.assertRaises(AccountNotFoundError):
            JournalEntryService.create_entry(
                entry_type='general', date='2024-01-15', description='اختبار', lines=lines, created_by=self.user
            )
...
if __name__ == '__main__':
    ...
    failures = test_runner.run_tests(['accounts.tests', 'purchases.tests', 'sales.tests'])
    exit(failures)
