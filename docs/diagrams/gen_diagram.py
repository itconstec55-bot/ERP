import os
from reportlab.graphics.shapes import Drawing, Rect, String, Line, Path, Polygon
...
for i in range(len(boxes) - 1):
    ...
    d.add(Polygon([W / 2 - 6, y2 - 2, W / 2 + 6, y2 - 2, W / 2, y2 + 8],
                  fillColor=colors.HexColor('#33475b'), strokeColor=colors.HexColor('#33475b')))
...
for top_y, lbl, sub, fill, tcol in boxes:
    ...
    d.add(String(W / 2, y + BH / 2 + 7, lbl, fontSize=15, fillColor=tcol, textAnchor='middle', fontName='Helvetica-Bold'))
...
y8 = ry(658, 28)
y1 = ry(56, 28)
d.add(Line(xr, y8, xr, y1, strokeColor=colors.HexColor('#1cc88a'),
           strokeWidth=2.5, strokeDashArray=[6, 4]))
# arrowhead at top (pointing up toward box1)
d.add(Polygon([xr - 6, y1 + 2, xr + 6, y1 + 2, xr, y1 - 8],
              fillColor=colors.HexColor('#1cc88a'), strokeColor=colors.HexColor('#1cc88a')))

d.add(String(W / 2, 30, 'Data Flow: Browser <-> Django Backend', fontSize=18,
             fillColor=colors.HexColor('#222222'), textAnchor='middle', fontName='Helvetica-Bold'))
