    ...
    if os.path.exists(img_path):
        if img_path.lower().endswith('.svg'):
            ...
            if svg_to_png(img_path, png_path, width_pt):
                ...
                return Table([[Paragraph('[صورة: ' + os.path.basename(img_path) + ']', codest)]], colWidths=[width_pt])
        try:
...
body = ParagraphStyle(
    'body', fontName='Arabic', fontSize=10.5, leading=16, alignment=TA_RIGHT, wordWrap='RTL', firstLineIndent=0
)
h1 = ParagraphStyle(
    'h1', parent=body, fontName='Arabic-Bold', fontSize=16, leading=22, spaceBefore=12, spaceAfter=6, alignment=TA_RIGHT
)
h2 = ParagraphStyle(
    'h2', parent=body, fontName='Arabic-Bold', fontSize=13, leading=19, spaceBefore=9, spaceAfter=4, alignment=TA_RIGHT
)
h3 = ParagraphStyle(
    'h3',
    parent=body,
...
quote = ParagraphStyle(
    'quote', parent=body, leftIndent=12, rightIndent=12, backColor=colors.HexColor('#eef2f7'), borderPadding=5
)
cellst = ParagraphStyle('cell', parent=body, fontSize=8.6, leading=12, alignment=TA_RIGHT, wordWrap='RTL')
codest = ParagraphStyle(
    'code',
    fontName='Arabic',
    fontSize=8,
    leading=11,
    ...
    while i < n:
        ...
        if line.strip().startswith('```'):
            if not in_code:
                ...
                txt = '\n'.join(code_buf)
                t = Table(
                    [[Paragraph(txt.replace('&', '&amp;').replace('<', '&lt;').replace('>', '&gt;'), codest)]],
                    colWidths=[AVAIL],
                )
                t.setStyle(
                    TableStyle(
