...
class BankReconciliationImportTest(TestCase):
    ...
    def _upload_csv(self, content):
        ...
        return self.client.post(
            '/bank-reconciliation/import-csv/',
            {
                ...
                'csv_file': SimpleUploadedFile('stmt.csv', content.encode('utf-8-sig'), content_type='text/csv'),
            },
        )
    ...
    def test_import_csv_skips_negative_amounts(self):
        csv_content = 'date,description,reference,debit,credit\n2026-01-07,قيمة سالبة,REF3,-50,0\n'
        resp = self._upload_csv(csv_content)
        self.assertEqual(resp.status_code, 302)
...
class BankItemMatchTest(TestCase):
    def setUp(self):
        ...
        self.tx_other = BankTransaction.objects.create(
            ...
            date=date(2026, 1, 10),
            amount=Decimal('500'),
            description='معاملة أخرى',
        )

    def test_match_same_bank_succeeds(self):
        resp = self.client.post(
            f'/bank-reconciliation/items/{self.item.pk}/match/', {'transaction_id': str(self.tx_same.pk)}
        )
        self.assertEqual(resp.status_code, 302)
    ...
    def test_match_different_bank_rejected(self):
        resp = self.client.post(
            f'/bank-reconciliation/items/{self.item.pk}/match/', {'transaction_id': str(self.tx_other.pk)}
        )
        self.assertEqual(resp.status_code, 302)
        self.item.refresh_from_db()
