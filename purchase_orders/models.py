...
class PurchaseOrder(models.Model):
    ...
    cost_center = models.ForeignKey(CostCenter, on_delete=models.SET_NULL, null=True,
                                    blank=True, verbose_name='مركز التكلفة')
    ...
    def budget_check(self):
        ...
        if not self.cost_center:
            return {'ok': True, 'available': None,
                    'message': 'لم يُحدَّد مركز تكلفة — لا يوجد قيد موازنة'}
        ...
        if total <= available:
            return {'ok': True, 'available': available,
                    'message': 'الطلب ضمن الموازنة المتاحة'}
        return {'ok': False, 'available': available,
                'message': 'الطلب يتجاوز الموازنة المتاحة لمركز التكلفة'}


class PurchaseOrderLine(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    order = models.ForeignKey(PurchaseOrder, on_delete=models.CASCADE,
                              related_name='lines', verbose_name='أمر الشراء')
    ...
    received_quantity = models.DecimalField(max_digits=20, decimal_places=10, default=0,
