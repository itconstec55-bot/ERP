...
from treasury.models import Bank, BankTransaction, Safe


@pytest.mark.django_db
class TestBank:
    def test_create_bank(self):
        bank = Bank.objects.create(name='بنك القاهرة', account_number='123456', current_balance=Decimal('10000.00'))
...
class TestSafe:
    def test_create_safe(self):
        safe = Safe.objects.create(name='الخزينة الرئيسية', current_balance=Decimal('50000.00'))
...
class TestBankTransaction:
    def test_create_deposit(self):
        ...
        tx = BankTransaction.objects.create(
            bank=bank, transaction_type='deposit', amount=Decimal('5000.00'), description='إيداع'
        )
    ...
    def test_transaction_type_choices(self):
        ...
        tx = BankTransaction.objects.create(
            bank=bank, transaction_type='withdrawal', amount=Decimal('1000.00'), description='سحب'
        )
