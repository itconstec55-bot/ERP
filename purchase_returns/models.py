...
class PurchaseReturn(models.Model):
    ...
    original_invoice = models.ForeignKey('purchases.PurchaseInvoice', on_delete=models.SET_NULL,
                                         null=True, blank=True, verbose_name='فاتورة الشراء الأصلية')
    ...
    def create_journal_entry(self):
        ...
        lines = [
            {'account': supplier_account, 'debit': self.total_amount, 'credit': 0,
             'description': f'مرتجع مشتريات - {self.supplier.name}'},
            {'account': purchases_account, 'debit': 0, 'credit': self.subtotal,
             'description': f'مشتريات مرتجعة - {self.return_number}'},
        ]
        if self.vat_amount > 0:
            lines.append({'account': JournalEntryService.get_account('1350'), 'debit': 0,
                          'credit': self.vat_amount, 'description': f'ضريبة مرتجع مشتريات - {self.return_number}'})
