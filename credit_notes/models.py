...
class CreditNote(models.Model):
    TYPE_CHOICES = [
        ('credit_note', 'إشعار دائن (مرتجع مبيعات)'),
        ('debit_note', 'إشعار مدين (مرتجع مشتريات)'),
    ]
    STATUS_CHOICES = [
        ('draft', 'مسودة'),
        ('posted', 'مرحل'),
    ...
    original_sales_invoice = models.ForeignKey(
        ...
        verbose_name='فاتورة المبيعات الأصلية'
    )
    original_purchase_invoice = models.ForeignKey(
        'purchases.PurchaseInvoice', on_delete=models.SET_NULL, null=True, blank=True,
