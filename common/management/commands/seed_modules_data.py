...
class Command(BaseCommand):
    ...
    def make_banks_safes(self):
        ...
        def run():
            ...
            b1 = Bank.objects.create(name='البنك الأهلي المصري', branch='فرع وسط البلد',
                                     account_number='1001123456', iban='EG123456', swift_code='NBEGEGCX',
                                     account=bank_acc, current_balance=Decimal('850000.00'))
            b2 = Bank.objects.create(name='بنك CIB', branch='فرع التجمع',
                                     account_number='2002987654', iban='EG654321', swift_code='CIBEEGCX',
                                     account=bank_acc, current_balance=Decimal('430000.00'))
            Safe.objects.create(name='خزينة الإدارة', responsible_person='أحمد محاسب',
                                account=cash, current_balance=Decimal('120000.00'), maximum_limit=Decimal('500000.00'))
            Safe.objects.create(name='خزينة الموقع', responsible_person='محمود أمين',
                                account=cash, current_balance=Decimal('60000.00'), maximum_limit=Decimal('300000.00'))
    ...
    def make_customers_products(self):
        ...
        def run():
            ...
            if Customer.objects.count() < 6:
                names = ['شركة الأفق للمقاولات', 'مؤسسة البناء الحديث', 'مجموعة النيل العقارية',
                         'شركة الرواد للتطوير', 'مصنع الخرسانة الجاهزة', 'مشروع برج النيل']
                for i, n in enumerate(names, 1):
                    Customer.objects.get_or_create(name=n, defaults={
                        'code': f'C-2026-{i:03d}', 'account': cust_acc,
                        'phone': f'01{random.randint(100000000,999999999)}',
                    })
            if Supplier.objects.count() < 6:
                names = ['شركة أسمنت العامرية', 'مصانع الرمل والركام', 'شركة كيماويات البناء',
                         'مؤسسة نقل المواد', 'شركة الحديد والصلب', 'مورد المضخات']
                for i, n in enumerate(names, 1):
                    Supplier.objects.get_or_create(name=n, defaults={
                        'code': f'S-2026-{i:03d}', 'account': supp_acc,
                        'phone': f'01{random.randint(100000000,999999999)}',
                    })
            if Product.objects.filter(name__icontains='خرسانة').count() == 0:
                Product.objects.get_or_create(name='خرسانة جاهزة C25/30', defaults={
                    'code': 'CON-C25', 'unit': 'م3', 'purchase_price': Decimal('420.00'),
                    'selling_price': Decimal('560.00')})
                Product.objects.get_or_create(name='خرسانة جاهزة C30/37', defaults={
                    'code': 'CON-C30', 'unit': 'م3', 'purchase_price': Decimal('450.00'),
        ...
        def run():
            ...
            for name, code, unit, pp, up in base:
                Product.objects.get_or_create(name=name, defaults={
                    'code': code, 'unit': unit, 'purchase_price': pp,
                    'selling_price': up})
            ...
            if Currency.objects.count() == 0:
                ...
                for code, name, sym, rate, is_base in base:
                    Currency.objects.create(
                        ...
                        exchange_rate_to_egp=rate, is_base=is_base)
        self.d('العملات', run)
    ...
    def make_warehouses(self):
        ...
        def run():
            if Warehouse.objects.count() == 0:
                Warehouse.objects.create(code='WH-01', name='مخزن المواد الخام',
                                         location='المصنع الرئيسي', manager='سعيد')
                Warehouse.objects.create(code='WH-02', name='مخزن قطع الغيار',
                                         location='ورشة الصيانة', manager='وليد')
                Warehouse.objects.create(code='WH-03', name='مخزن المنتجات النهائية',
                                         location='الموقع', manager='طارق')
                ...
                for wh in Warehouse.objects.all()[:3]:
                    for p in products[:6]:
                        ...
                        StockMovement.objects.create(
                            ...
                            movement_type='in', warehouse=wh, product=p,
                            quantity=wp.quantity, unit_cost=wp.quantity * p.purchase_price,
    ...
    def make_concrete_production(self):
        from concrete_production.models import (
            ...
            Truck, ProductionBatch, DeliverySchedule, ProductionCost, Silo, SiloTransaction)
        from sales.models import Customer
        from purchases.models import Product
        ...
        def run():
            ...
            if ConcreteMixDesign.objects.count() == 0:
                ...
                for code, name, sc, slump, agg, wcr, ts in designs:
                    md = ConcreteMixDesign.objects.create(
                        code=code, name=name, strength_class=sc, slump_cm=slump,
                        max_aggregate_mm=agg, water_cement_ratio=wcr,
                        target_strength_mpa=ts, selling_price_per_m3=Decimal('560.00'),
                    ...
                    for i, (ct, nm, prod, qty) in enumerate(comps, 1):
                        MixComponent.objects.create(mix_design=md, component_type=ct, name=nm,
                                                    quantity_kg=qty, product=prod, order=i)
                    md.calculate_cost()

            # أسطول الشاحنات
            if Truck.objects.count() == 0:
                for i, plate in enumerate(['ط ا ب 1234', 'ط ا ب 5678', 'ط ج د 9012', 'ط و ز 3456'], 1):
                    Truck.objects.create(plate_number=plate, driver_name=f'سائق {i}',
                                         driver_phone=f'01{random.randint(100000000,999999999)}',
                                         capacity_m3=random.choice([8, 9, 10]), status='available')
            ...
            if CompanyBranch.objects.count() == 0 and Company.objects.exists():
                branch = CompanyBranch.objects.create(
                    company=Company.objects.first(), name='الفرع الرئيسي',
            ...
            for i in range(1, 9):
                cr = CustomerRequest.objects.create(
                    ...
                    contact_person=f'مهندس {i}', contact_phone=f'01{random.randint(100000000,999999999)}',
                    status=random.choice(statuses_cr), created_by=self.admin)
                md = random.choice(designs)
                po = ProductionOrder.objects.create(
                    customer_request=cr, mix_design=md,
                    ...
                    unit_price=md.selling_price_per_m3, created_by=self.admin,
                    branch=branch if i % 2 == 0 else None)
                # دفعة + تسليم
                if po.status in ['in_progress', 'completed']:
                    for j in range(1, random.randint(2, 4)):
                        batch = ProductionBatch.objects.create(
                            production_order=po, truck=random.choice(trucks),
        ...
        def run():
            ...
            if Contractor.objects.count() == 0:
                ...
                for i, (name, ctype, spec, cl) in enumerate(contractor_data, 1):
                    Contractor.objects.create(
                        code=f'CONT-2026-{i:03d}', name=name, contractor_type=ctype,
                        tax_number=f'2{random.randint(100000000,999999999)}',
                        phone=f'01{random.randint(100000000,999999999)}',
                        email=f'contractor{i}@example.com', address=f'عنوان {i}',
                        speciality=spec, credit_limit=cl, retention_rate=Decimal('5'),
            ...
            for idx, cont in enumerate(contractors, 1):
                ...
                contract = Contract.objects.create(
                    contractor=cont, title=f'عقد أعمال {cont.name}',
                    contract_type='lump_sum', status='active',
                    contract_amount=Decimal(random.randint(200000, 1500000)),
                    vat_rate=Decimal('14'), signing_date=self.today - timedelta(days=120),
                    start_date=self.today - timedelta(days=100),
                    end_date=self.today + timedelta(days=200),
                    retention_rate=Decimal('5'), advance_payment_percent=Decimal('10'),
                    cost_account=cost_acc, created_by=self.admin)
                ...
                for j, (desc, unit, qty, price) in enumerate(items_data, 1):
                    ci = ContractItem.objects.create(
                        contract=contract, item_number=f'IT-{j}', description=desc,
                        unit=unit, quantity=qty, unit_price=price, order=j)
                ...
                for k in range(1, 4):
                    cert = InterimCertificate.objects.create(
                        contract=contract, period_number=k, status='paid',
                    ...
                    for ci in items:
                        ...
                        CertificateItem.objects.create(
                            certificate=cert, contract_item=ci,
                    ...
                    ContractorPayment.objects.create(
                        contract=contract, certificate=cert,
                        amount=cert.net_amount * Decimal('0.9'),
                        payment_method='bank_transfer',
                        payment_date=cert.payment_date, status='paid',
                        created_by=self.admin).create_journal_entry()
        ...
        def run():
            ...
            for i in range(1, 7):
                Cheque.objects.create(
                    cheque_number=f'CHK-R-{i:05d}', cheque_type='received',
            ...
            for i in range(1, 7):
                Cheque.objects.create(
                    cheque_number=f'CHK-I-{i:05d}', cheque_type='issued',
                    bank_name=random.choice(['البنك الأهلي', 'CIB', 'بنك مصر']),
        ...
        def run():
            ...
            for i in range(1, 8):
                ...
                pr = PaymentReceipt.objects.create(
                    receipt_number=f'PR-REC-{i:05d}', receipt_type='receipt',
            ...
            for i in range(1, 8):
                ...
                pr = PaymentReceipt.objects.create(
                    receipt_number=f'PR-PAY-{i:05d}', receipt_type='payment',
                    date=self.today - timedelta(days=random.randint(1, 30)),
        ...
        def run():
            ...
            for i in range(1, 6):
                ...
                sr = SalesReturn.objects.create(
                    ...
                    customer=cust, original_invoice=inv,
                    reason='عيوب تصنيع / كسر أثناء النقل', created_by=self.admin)
                p = random.choice(products) if products else None
                if p:
                    SalesReturnLine.objects.create(
                        sales_return=sr, product=p, quantity=Decimal(random.randint(1, 5)),
    ...
    def make_purchase_returns(self):
        ...
        from purchases.models import Supplier, PurchaseInvoice, Product
        ...
        def run():
            ...
            for i in range(1, 6):
                ...
                pr = PurchaseReturn.objects.create(
                    ...
                    date=self.today - timedelta(days=random.randint(1, 25)),
                    supplier=supp, original_invoice=inv,
                    reason='مواد غير مطابقة للمواصفات', created_by=self.admin)
                p = random.choice(products) if products else None
                if p:
                    PurchaseReturnLine.objects.create(
                        purchase_return=pr, product=p, quantity=Decimal(random.randint(1, 5)),
        ...
        def run():
            ...
            for i, wh in enumerate(warehouses, 1):
                adj = StockAdjustment.objects.create(
                    ...
                    adjustment_type=random.choice(['addition', 'count']), warehouse=wh,
                    reason='جرد دوري وتصحيح أرصدة', status='draft',
                    created_by=self.admin)
                for p in products[:4]:
                    StockAdjustmentLine.objects.create(
                        adjustment=adj, product=p,
    ...
    def make_budget(self):
        ...
        def run():
            if CostCenter.objects.count() == 0:
                parent = CostCenter.objects.create(code='CC-00', name='المركز الرئيسي',
                                                    description='الإدارة العامة', manager='المدير المالي')
                subs = [('CC-01', 'مصنع الخرسانة'), ('CC-02', 'قسم المقاولات'),
                        ('CC-03', 'قسم المبيعات'), ('CC-04', 'قسم الصيانة')]
                for code, name in subs:
                    CostCenter.objects.create(code=code, name=name, parent=parent,
                                              description=f'مركز تكلفة {name}')
                ...
                for cc in centers:
                    for acc in accounts:
                        Budget.objects.create(
                            ...
                            account=acc, cost_center=cc, period='monthly',
                            year=2026, month=7,
        ...
        def run():
            ...
            for i in range(1, 5):
                ...
                CreditNote.objects.create(
                    ...
                    date=self.today - timedelta(days=random.randint(1, 20)),
                    customer=cust, original_sales_invoice=inv,
                    subtotal=sub, vat_amount=vat, total_amount=sub + vat,
                    reason='إشعار دائن لمرتجع مبيعات', is_posted=True)
            for i in range(1, 5):
                supp = random.choice(suppliers) if suppliers else None
                ...
                CreditNote.objects.create(
                    ...
                    date=self.today - timedelta(days=random.randint(1, 20)),
                    supplier=supp, original_purchase_invoice=inv,
                    subtotal=sub, vat_amount=vat, total_amount=sub + vat,
                    reason='إشعار مدين لمرتجع مشتريات', is_posted=True)
            ...
            for cur in Currency.objects.exclude(is_base=True):
                for d in range(1, 7):
                    ExchangeRateHistory.objects.get_or_create(
                        currency=cur, date=base - timedelta(days=d * 5),
                        defaults={
