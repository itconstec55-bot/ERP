...
def calculate_invoice_totals(
    lines: list[dict[str, Any]], discount_amount: Number = 0, withholding_rate: Number = 0, is_tax_invoice: bool = True
) -> dict[str, Decimal]:
    """
...
def validate_invoice_precision(invoice) -> list[str]:
    ...
    if invoice.withholding_tax_amount != calculated['withholding_amount']:
        errors.append(
            f'الخصم والتحصيل غير متطابق: المخزن={invoice.withholding_tax_amount}, المحسوب={calculated["withholding_amount"]}'
        )
