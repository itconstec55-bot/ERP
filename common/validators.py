...
def validate_positive_decimal(value):
    ...
    if value is not None and value <= Decimal('0'):
        raise ValidationError(
            _('القيمة يجب أن تكون أكبر من صفر'),
            params={'value': value},
        )
...
def validate_non_negative_decimal(value):
    ...
    if value is not None and value < Decimal('0'):
        raise ValidationError(
            _('القيمة لا يمكن أن تكون سالبة'),
            params={'value': value},
        )
...
def validate_balanced_entry(debit, credit):
    ...
    if debit != credit:
        raise ValidationError(
            _('القيد غير متوازن: مدين=%(debit)s، دائن=%(credit)s'),
...
def validate_vat_rate(value):
    ...
    if value is not None and (value < Decimal('0') or value > Decimal('100')):
        raise ValidationError(
            _('نسبة الضريبة يجب أن تكون بين 0 و 100'),
            params={'value': value},
        )
...
def validate_payment_method(value):
    ...
    if value not in valid_methods:
        raise ValidationError(
            _('طريقة دفع غير صالحة. القيم المسموحة: %(valid)s'),
            params={'valid': ', '.join(valid_methods)},
        )
...
def validate_supplier_type(value):
    ...
    if value not in valid_types:
        raise ValidationError(
            _('نوع المورد غير صالح. القيم المسموحة: %(valid)s'),
            params={'valid': ', '.join(valid_types)},
        )
...
def validate_customer_type(value):
    ...
    if value not in valid_types:
        raise ValidationError(
            _('نوع العميل غير صالح. القيم المسموحة: %(valid)s'),
            params={'valid': ', '.join(valid_types)},
        )
...
def validate_entry_type(value):
    ...
    """
    valid_types = [
        'general', 'purchase', 'sale', 'receipt', 
        'payment', 'depreciation', 'payroll', 'adjustment'
    ]
    if value not in valid_types:
        raise ValidationError(
            _('نوع القيد غير صالح. القيم المسموحة: %(valid)s'),
            params={'valid': ', '.join(valid_types)},
        )
...
class FinancialValidator:
    ...
    @staticmethod
    def validate_invoice_totals(subtotal, vat_amount, discount_amount, 
        ...
        if total_amount != expected_total:
            raise ValidationError(
                f'الإجمالي غير صحيح. المتوقع: {expected_total}, المدخل: {total_amount}'
            )
    ...
    def validate_invoice_payment(payment_amount, remaining_amount):
        ...
        if payment_amount > remaining_amount:
            raise ValidationError(
                f'مبلغ الدفع ({payment_amount}) أكبر من المبلغ المتبقي ({remaining_amount})'
            )
    ...
    def validate_stock_availability(product, requested_quantity, warehouse=None):
        ...
        if warehouse:
            available = WarehouseProduct.objects.filter(
        ...
        if available < requested_quantity:
            raise ValidationError(
                f'مخزون غير كافٍ. المتوفر: {available}, المطلوب: {requested_quantity}'
            )