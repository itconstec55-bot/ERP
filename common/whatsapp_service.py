...
class WhatsAppAPIResponse:
    ...
    retry_after: Optional[int] = None  # ثوانٍ للانتظار قبل إعادة المحاولة
...
class WhatsAppRateLimiter:
    ...
    def check_limit(self, phone: str = None) -> tuple[bool, Optional[int]]:
        """
...
class WhatsAppService:
    ...
    def format_invoice_message(invoice, invoice_type: str = "sales") -> str:
        ...
        from common.decimal_utils import quantize_10, quantize_display
    ...
    def send_with_retry(self, phone: str, message: str, max_retries: int = 3) -> WhatsAppAPIResponse:
        ...
        if not normalized:
            return WhatsAppAPIResponse(
                success=False,
                error_code=WhatsAppErrorCode.INVALID_PHONE,
                error_message="رقم هاتف غير صالح"
            )
    ...
    def _send_via_api(self, phone: str, message: str) -> WhatsAppAPIResponse:
        ...
        try:
            ...
            return WhatsAppAPIResponse(
                success=False,
                error_code=WhatsAppErrorCode.NETWORK_ERROR,
                error_message=f"خطأ شبكة: {str(e)}"
            )
