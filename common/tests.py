...
from concrete_production.models import (
    Silo, SiloTransaction, ConcreteMixDesign, MixComponent,
    CustomerRequest, ProductionOrder,
)
...
class TestSiloTransactions(TestCase):
    ...
    def setUp(self):
        self.silo = Silo.objects.create(
            name='سيلة 1', code='S-T1', capacity_tons=Decimal('100'),
            current_stock_tons=Decimal('50'), cement_type='CEM I')

    def test_increases_stock(self):
        before = self.silo.current_stock_tons
        SiloTransaction.objects.create(
            silo=self.silo, transaction_type='in',
            quantity_tons=Decimal('10'), reference_number='T1',
            notes='توريد')
    ...
    def test_decreases_stock(self):
        before = self.silo.current_stock_tons
        SiloTransaction.objects.create(
            silo=self.silo, transaction_type='out',
            quantity_tons=Decimal('5'), reference_number='T2')
        self.silo.refresh_from_db()
...
class TestProductionOrderModel(TestCase):
    ...
    def setUp(self):
        ...
        self.mix = ConcreteMixDesign.objects.create(
            code='C25', name='C25/30', strength_class='C25/30',
            slump_cm=Decimal('12'), max_aggregate_mm=Decimal('20'),
            water_cement_ratio=Decimal('0.5'), target_strength_mpa=Decimal('25'),
            selling_price_per_m3=Decimal('560.00'))
        self.cr = CustomerRequest.objects.create(
            request_number='T-CR-1', customer=self.customer,
            project_name='مشروع اختبار', site_address='عنوان')

    def test_total_price_computed(self):
        po = ProductionOrder.objects.create(
            customer_request=self.cr, mix_design=self.mix,
            quantity_m3=Decimal('100'), unit_price=Decimal('560.00'))
...
class TestConcreteAutoInvoice(TestCase):
    ...
    def setUp(self):
        ...
        self.product = Product.objects.create(
            name='خرسانة جاهزة C25/30', code='T-CON', unit='م3',
            purchase_price=Decimal('420.00'))
        self.mix = ConcreteMixDesign.objects.create(
            code='C25', name='C25/30', strength_class='C25/30',
            slump_cm=Decimal('12'), max_aggregate_mm=Decimal('20'),
            water_cement_ratio=Decimal('0.5'), target_strength_mpa=Decimal('25'),
            selling_price_per_m3=Decimal('560.00'), product=self.product)
        self.cr = CustomerRequest.objects.create(
            request_number='T-CR-2', customer=self.customer,
            project_name='مشروع اختبار', site_address='عنوان')
    ...
    def test_no_duplicate_invoice(self):
        po = ProductionOrder.objects.create(
            customer_request=self.cr, mix_design=self.mix,
            quantity_m3=Decimal('50'), unit_price=Decimal('560.00'),
            status='completed')
        ...
        from sales.models import SalesInvoice
        self.assertEqual(
            SalesInvoice.objects.filter(production_order=po).count(), 1)
    ...
    def test_detail_page_shows_invoice(self):
        ...
        resp = client.get(reverse(
            'concrete_production:production_order_detail', args=[po.pk]))
...
class TestStockAdjustmentApprove(TestCase):
    ...
    def setUp(self):
        self.product = Product.objects.create(
            name='منتج اختبار', code='T-PROD', unit='طن',
            purchase_price=Decimal('100'))
        self.warehouse = Warehouse.objects.create(
            code='T-WH', name='مخزن اختبار')
    ...
    def test_addition_increases_stock(self):
        adj = StockAdjustment.objects.create(
            adjustment_number='T-SA-1', adjustment_type='addition',
            warehouse=self.warehouse, date=date.today(), reason='إضافة')
        StockAdjustmentLine.objects.create(
            adjustment=adj, product=self.product, quantity=Decimal('5'))
        adj.approve()
    ...
    def test_deduction_decreases_stock(self):
        adj = StockAdjustment.objects.create(
            adjustment_number='T-SA-2', adjustment_type='deduction',
            warehouse=self.warehouse, date=date.today(), reason='خصم')
        StockAdjustmentLine.objects.create(
            adjustment=adj, product=self.product, quantity=Decimal('4'))
        adj.approve()
        self.wp.refresh_from_db()
...
class TestViewsAccess(TestCase):
    ...
    def test_new_screens_200_for_admin(self):
        ...
        for name in ['reports:financial_dashboard', 'reports:workflow_tracker',
...
class TestSeedModulesData(TestCase):
    ...
    def test_seed_populates_all_modules(self):
        ...
        self.assertTrue(
            InterimCertificate.objects.filter(journal_entry__isnull=False).exists())
        self.assertTrue(
            ContractorPayment.objects.filter(journal_entry__isnull=False).exists())
...
class TestObjectLevelPermissions(TestCase):
    ...
    def setUp(self):
        ...
        self.branch_a = CompanyBranch.objects.create(
            company=company, name='الفرع أ', is_default=True)
        self.branch_b = CompanyBranch.objects.create(
            company=company, name='الفرع ب')
        ...
        UserProfile.objects.create(
            user=self.user_all, branch=self.branch_a, view_all_branches=True)
        ...
        for u in (self.user_a, self.user_b, self.user_all):
            UserScreenPermission.objects.create(
                user=u, screen=screen, grant_type='allow', can_view=True)

        self.customer = Customer.objects.create(name='عميل فرعي', code='OBJ-CUST')
        self.inv_a = SalesInvoice.objects.create(
            invoice_number='OBJ-A', customer=self.customer, date=date.today(),
            branch=self.branch_a)
        self.inv_b = SalesInvoice.objects.create(
            invoice_number='OBJ-B', customer=self.customer, date=date.today(),
            branch=self.branch_b)
        self.inv_none = SalesInvoice.objects.create(
            invoice_number='OBJ-N', customer=self.customer, date=date.today())

    def test_filter_by_branch(self):
        from common.permissions import filter_by_branch
        from sales.models import SalesInvoice
        self.assertEqual(
            filter_by_branch(SalesInvoice.objects.all(), self.user_a).count(), 2)
        self.assertEqual(
            filter_by_branch(SalesInvoice.objects.all(), self.user_b).count(), 2)
        self.assertEqual(
            filter_by_branch(SalesInvoice.objects.all(), self.user_all).count(), 3)
        self.assertEqual(
            filter_by_branch(SalesInvoice.objects.all(), self.superuser).count(), 3)

    def test_has_object_permission(self):
        from common.permissions import has_object_permission
        self.assertTrue(has_object_permission(
            self.user_a, 'sales.view_salesinvoice', self.inv_a))
        self.assertFalse(has_object_permission(
            self.user_a, 'sales.view_salesinvoice', self.inv_b))
        self.assertTrue(has_object_permission(
            self.user_a, 'sales.view_salesinvoice', self.inv_none))
        self.assertTrue(has_object_permission(
            self.user_all, 'sales.view_salesinvoice', self.inv_b))
        self.assertFalse(has_object_permission(
            self.user_b, 'sales.view_salesinvoice', self.inv_a))
