...
def global_search(request):
    ...
    for a in accounts.order_by('code')[:limit]:
        results.append({'type': 'حساب', 'name': a.name, 'code': a.code,
                        'url': f'/accounts/account/{a.id}/'})
    ...
    for p in products.order_by('code')[:limit]:
        results.append({'type': 'منتج', 'name': p.name, 'code': p.code,
                        'url': f'/purchases/products/'})
    ...
    for c in customers.order_by('code')[:limit]:
        results.append({'type': 'عميل', 'name': c.name, 'code': c.code,
                        'url': f'/sales/customers/'})
    ...
    for s in suppliers.order_by('code')[:limit]:
        results.append({'type': 'مورد', 'name': s.name, 'code': s.code,
                        'url': f'/purchases/suppliers/'})
...
def whatsapp_webhook(request):
    ...
    if not expected_secret or not hmac.compare_digest(
        token, 'sha256=' + hmac.new(
            expected_secret.encode(), request.body, hashlib.sha256
        ).hexdigest()
    ):
