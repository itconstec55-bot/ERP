...
class JournalEntryService:
    ...
    @transaction.atomic
    def create_entry(
        entry_type, date, description, reference='', lines=None, created_by=None, entry_number=None, file_number=None
    ):
        if lines is None:
            lines = []
        ...
        if total_debit != total_credit:
            raise UnbalancedEntryError(f'القيد غير متوازن: مدين={total_debit}, دائن={total_credit}')
        ...
        account_codes = [
            line['account'].code if hasattr(line['account'], 'code') else line['account'] for line in lines
        ]
        ...
        logger.info('Journal entry created: type=%s ref=%s total=%.2f', entry_type, reference, total_debit)
        return entry
